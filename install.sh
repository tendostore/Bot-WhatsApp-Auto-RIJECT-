...
async function startBot() {
    ...
    sock.ev.on('call', async (callList) => {
        for (const call of callList) {
            ...
            const callerJid = call.from; // JID asli dari event call, dipakai untuk rejectCall
            console.log(`[debug] Panggilan masuk, JID: ${callerJid}`);

            // WhatsApp bisa mengirim identitas panggilan sebagai @lid (privasi nomor
            // telepon aktif). JID @lid TIDAK bisa dipakai langsung untuk sendMessage
            // (pesan akan dianggap "terkirim" oleh Baileys tapi tidak sampai ke penerima),
            // jadi harus di-resolve dulu ke JID nomor asli (@s.whatsapp.net).
            let messagingJid = callerJid;
            if (callerJid.endsWith('@lid')) {
                try {
                    const resolvedPN = await sock.signalRepository.lidMapping.getPNForLID(callerJid);
                    if (resolvedPN) {
                        messagingJid = resolvedPN;
                        console.log(`[debug] LID ${callerJid} berhasil di-resolve ke ${resolvedPN}`);
                    } else {
                        console.log(`[debug] LID ${callerJid} belum bisa di-resolve ke nomor asli (belum ada mapping tersimpan). Balasan mungkin tidak terkirim.`);
                    }
                } catch (e) {
                    console.log(`[debug] Gagal resolve LID ${callerJid}:`, e.message);
                }
            }
            const callerNumber = messagingJid.split('@')[0].split(':')[0];
            ...
            try {
                ...
                const now = Date.now();
                const last = lastReplyAt.get(messagingJid) || 0;
                if (now - last >= REPLY_COOLDOWN_MS) {
                    ...
                    const sendResult = await sock.sendMessage(messagingJid, { text: pesan });
                    console.log(`[debug] sendMessage ke ${messagingJid}:`, sendResult ? 'terkirim (ada message ID)' : 'TIDAK ADA HASIL (kemungkinan gagal diam-diam)');
                    lastReplyAt.set(messagingJid, now);
                } else {
...
echo -e "${KUNING}[*] Menginstal library Baileys 7.0.0-rc13 (dengan dukungan resolusi @lid)...${NORMAL}"
